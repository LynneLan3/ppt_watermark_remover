...
import { notebooklmSeoPageContent } from "@/content/pages/notebooklm-watermark-remover";
...
export default function NotebooklmWatermarkRemoverPage() {
  const content = notebooklmSeoPageContent;
  ...
  return (
    <div className="min-h-screen bg-slate-100">
      ...
      <section className="px-4 py-6 sm:px-6 sm:py-8">
        <ContentGridBlock
          title={content.whyCleanupMatters.title}
          intro={content.whyCleanupMatters.intro}
          items={content.whyCleanupMatters.items}
        />
      </section>

      <section className="px-4 py-6 sm:px-6 sm:py-8">
        <ContentGridBlock
          title={content.commonUseCases.title}
          intro={content.commonUseCases.intro}
          items={content.commonUseCases.items}
        />
      </section>

      <section className="px-4 py-6 sm:px-6 sm:py-8">
        <ContentGridBlock
          title={content.whyManualCleanupSlow.title}
          intro={content.whyManualCleanupSlow.intro}
          items={content.whyManualCleanupSlow.items}
        />
      </section>

      <section id="how-the-tool-works" className="px-4 py-6 sm:px-6 sm:py-8">
        <div className="mx-auto max-w-6xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <h2 className="text-2xl font-semibold text-slate-900 sm:text-3xl">
            {content.howToolWorks.title}
          </h2>
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">
            {content.howToolWorks.intro}
          </p>
          <ol className="mt-6 grid gap-4 sm:grid-cols-3">
            {content.howToolWorks.steps.map((step) => (
